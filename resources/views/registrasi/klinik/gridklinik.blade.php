...
<tr>
    ...
    <td class="text-center">
        <button class="btn btn-outline-info p-1 mb-1" data-toggle="modal" data-target="#mdlDetailPemeriksaan" onclick="getDetailPemeriksaan({{ $val->id }}, {{ $val->jenis_lab_id }}, '')">Lihat Pemeriksaan</button>
    </td>
    ...
    <td class="text-center">
        <a href="/pencetakan/cetakbuktiregistrasi/{{$val->id}}" title="Cetak Bukti Registrasi" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="blue" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-printer"><polyline points="6 9 6 2 18 2 18 9"></polyline><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path><rect x="6" y="14" width="12" height="8"></rect></svg></a>&nbsp;
        @if (!$val->status_bayar)
            <button type="button" class="btn btn-danger btn-sm p-1" onclick="hapusRegistrasi({{$val->id}})">Hapus</button>
            <a href="/registrasi/klinik/edit/{{$val->id}}" class="btn btn-warning btn-sm p-1">Edit</a>
        @endif
