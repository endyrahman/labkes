        ...
        <table id="dt-pembayaran-verifikasi" class="table dt-table-hover" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:right;">{{ number_format($val->nominal_transfer, '0', ',', '.') }}</td>
                    <td>
                        @if (!$val->status_bayar)
                    ...
                    @if ($val->status_bayar == 1)
                        <a href="javascript:void(0);" onclick="mdlValidasiBayar({{$val->pemeriksaan_id}}, {{$val->pembayaran_id}}, {{$val->nominal_transfer}}, '{{$val->bukti_bayar}}' )" class="btn btn-success p-1">Validasi</a>
                    @endif
                    </td>
                </tr>
                @endforeach
